...
export function stageMetric(db: Database.Database, m: MetricInput): number {
  ...
  const row = { notebooklm_source_id: null, company_id: null, ...m };
...
export function promoteMetric(db: Database.Database, stagingId: number, trust: Trust = "verified"): number {
  ...
  const clean = { filing_id: row.filing_id, company_id: null, name: row.name, value: row.value, unit: row.unit, period: row.period, source_page: row.source_page, trust };
  const tx = db.transaction(() => {
    const info = db.prepare(
      `INSERT INTO metrics (filing_id, company_id, name, value, unit, period, source_page, trust)
       VALUES (@filing_id, @company_id, @name, @value, @unit, @period, @source_page, @trust)`,
    ).run(clean);
...
// Insert a screener-sourced metric directly (no staging/verification — deterministic scrape).
// Screener numbers skip the PDF gate; trust = 'screener'.
export function insertScreenerMetric(
  db: Database.Database,
  m: { company_id: number; name: string; value: number; unit: string | null; period: string; source_url: string | null },
): number {
  const info = db.prepare(
    `INSERT INTO metrics (filing_id, company_id, name, value, unit, period, source_page, trust)
     VALUES (NULL, @company_id, @name, @value, @unit, @period, NULL, 'screener')`,
  ).run(m);
  return Number(info.lastInsertRowid);
}
