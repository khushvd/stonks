// Shared mock data for the Stonks redesign — grounded in Indian Hotels (INDHOTEL)
// and its real hospitality peer set. Numbers are illustrative but realistic for FY24–FY26.
// "Trust = flag problems only": every figure is VERIFIED by default (no badge);
// only NLM-ONLY / REJECTED / CONTRADICTION are surfaced.
...
const STONKS = {
  ...
  // One-paragraph orientation for someone who's never looked at this company.
  about:
    "India's largest hospitality company (the Taj, SeleQtions, Vivanta, Ginger brands) — ~25,200 keys across luxury to economy, increasingly asset-light via management contracts. Listed, Tata group.",

  // The triage verdict — synthesized strictly from the figures below. Decision-support, not a call.
  bottomLine: {
  ...
  // The synthesized answer — the takeaway, answer-first.
  brief: {
    headline:
      "EBITDA margin has expanded ~8 pts over two years, from ~28% (Q1 FY24) to ~36% (Q4 FY25), with sharp seasonal peaks every Q3.",
    answer: [
  ...
  // Quarterly time series. period = short label; order is chronological.
  // margin = consolidated EBITDA margin (OPM %). rev/ebitda/pat in ₹ crore.
  quarters: [
  ...
  // Headline stat tiles (latest quarter vs prior-year quarter).
  stats: [
  ...
  // Peer KPI matrix. trust: 'ok' (silent), 'nlm' (NotebookLM-only, unverified),
  // 'rejected' (quarantined), 'missing' (gap / follow-up).
  ...
  // Small peer margin sparkline series (last 6 quarters, margin %) for in-cell trends.
  peerMargins: {
  ...
  // Management commentary trajectory — last 4 quarters. One contradiction flagged.
  commentary: [
    {
  ...
  // Source documents the analysis was built from.
  sources: [
  ...
  // Pipeline / run integrity — surfaced quietly, only problem counts loud.
  integrity: { verified: 47, nlmOnly: 3, pending: 2, rejected: 1 },
};

window.STONKS = STONKS;
